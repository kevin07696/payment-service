...
// TestAdminCLIStructure verifies AdminCLI struct can be instantiated
func TestAdminCLIStructure(t *testing.T) {
	t.Run("AdminCLI can be created with nil queries", func(t *testing.T) {
		cli := &AdminCLI{
			ctx:     nil,
			queries: nil,
		...
		assert.NotNil(t, cli)
		assert.Equal(t, "test-admin-id", cli.adminID)
	})
}
