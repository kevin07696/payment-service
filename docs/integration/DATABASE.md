...
| `transactions` | All payment operations | `parent_transaction_id` |
...
-- Get transaction chain (find all related transactions)
-- Method 1: Get all children of a specific transaction
SELECT * FROM transactions
WHERE parent_transaction_id = $1 AND deleted_at IS NULL
ORDER BY created_at;

-- Method 2: Get entire chain (parent + all descendants)
...
SELECT * FROM transaction_chain WHERE deleted_at IS NULL ORDER BY created_at;
