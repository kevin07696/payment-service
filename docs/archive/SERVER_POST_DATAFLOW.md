...
This document analyzes the complete dataflow for **Server POST** payment
transactions - direct server-to-server integration between calling services
(POS/e-commerce) and the Payment Service via gRPC. Server POST supports both
credit cards and ACH payments using saved payment methods or BRIC tokens.
