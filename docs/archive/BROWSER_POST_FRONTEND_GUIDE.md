...
Browser POST enables PCI-compliant **credit card payments** where card data
flows directly from browser to EPX, bypassing your backend entirely.

> **Payment Method Support**: Browser POST supports **credit cards only**.
> For ACH payments, use Server POST API.

**3-Step Integration**:

1. Backend calls Payment Service `GetPaymentForm` API → receives form config +
   transaction IDs
