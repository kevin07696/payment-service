...
│     - transaction_id, group_id, status                     │
│     - auth_code, auth_resp                                 │
└─────────────────────────────────────────────────────────────┘
```
...
// Response includes AUTH_GUID (BRIC token)
```

**Future Payments - Use BRIC:**
```go
request := &ServerPostRequest{
    TransactionType: TransactionTypeSale,
    Amount:          "150.00",
    AuthGUID:        "ACH-BRIC-TOKEN",  // Saved from first payment
}
```
